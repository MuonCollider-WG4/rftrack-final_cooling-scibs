...
os.makedirs("Phase_space_Bunch6d_SC_IBS", exist_ok=True)
...
for i, B_i in enumerate(S):
    ...
    np.savetxt(f"Phase_space_Bunch6d_SC_IBS/other_info_{i+1:03d}.txt",
               [other_info], delimiter="\t")

    np.savetxt(f"Phase_space_Bunch6d_SC_IBS/M_{i+1:03d}.txt",
               M_i, delimiter="\t")
...
np.savetxt("transport_table_3cells_SC_IBS.txt", T, fmt="%s")
